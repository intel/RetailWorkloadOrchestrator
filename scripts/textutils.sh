...
T_INFO_ICON="[${C_YELLOW}i${T_RESET}]"
...
printDatedMsg() {
  printMsg "${C_BLUE}$(date +"%Y-%m-%d %I:%M:%S")${T_RESET} - ${1}"
}

printDatedInfoMsg() {
  printMsg "${C_BLUE}$(date +"%Y-%m-%d %I:%M:%S")${T_RESET} - ${T_INFO_ICON} ${1}"
}
...
logInfoMsg() {
  echo "$(date +"%Y-%m-%d %I:%M:%S") ${T_INFO_ICON} ${1}" >>${LOG_FILE}
}
...
run() {
  ...
  if [ "$success" = false ]; then
    ...
    echo -e " ${T_OK_ICON} Success${T_RESET}" 2>&1 | tee -a /dev/tty0
  fi
}
